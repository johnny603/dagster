...
import {InstanceConcurrencyKeyInfo} from './InstanceConcurrencyKeyInfo';
...
export const InstanceConcurrencyPageContent = React.memo(() => {
  ...
  const currentPath: string[] = React.useMemo(
    () =>
      (currentPathStr || '')
        .split('/')
        .filter((x: string) => x)
  ...
  const concurrencyKey = currentPath.length && currentPath[0];
  if (!concurrencyKey) {
